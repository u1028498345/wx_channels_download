...
func (c *ChannelsClient) RequestFrontendWithIdleLiveClient(endpoint string, body interface{}, timeout time.Duration) (*ClientWebsocketResponse, error) {
	id := strconv.FormatUint(atomic.AddUint64(&c.req_seq, 1), 10)
	req := ClientWebsocketRequestBody{ID: id, Key: endpoint, Body: body}
	data, err := json.Marshal(APIClientWSMessage{Type: "api_call", Data: req})
	if err != nil {
		return nil, err
	}

	respChan := make(chan ClientWebsocketResponse, 1)
	c.requests_mu.Lock()
